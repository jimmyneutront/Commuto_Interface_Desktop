...
internal class CommutoCoreInteraction {
    ...
    fun parsePublicKeyAnnouncement(messageString: String, makerInterfaceId: ByteArray, offerId: ByteArray): PublicKey? {
        ...
        val publicKey = try {
            val publicKeyBytes = decoder.decode(payload.pubKey)
            val algorithmIdentifier: AlgorithmIdentifier = AlgorithmIdentifier(PKCSObjectIdentifiers.rsaEncryption, DERNull.INSTANCE);
            val pubKeyX509Bytes: ByteArray = SubjectPublicKeyInfo(algorithmIdentifier, publicKeyBytes).encoded
            val pubSecKey: java.security.PublicKey = KeyFactory.getInstance("RSA").generatePublic(X509EncodedKeySpec(pubKeyX509Bytes))
            PublicKey(pubSecKey)
        } catch (e: Exception) {
            return null
        }
    ...
    fun testPKAParsing() {
        ...
        val decoder = Base64.getDecoder()
        val pubKey: java.security.PublicKey = pubKeyFromPkcs1Bytes(decoder.decode(pubKeyB64))
        val privKey: PrivateKey = privKeyFromPkcs1Bytes(decoder.decode(privKeyB64))
        val javaSecKeyPair = java.security.KeyPair(pubKey, privKey)
    ...
    fun pubKeyFromPkcs1Bytes(pubKeyPkcs1Bytes: ByteArray): java.security.PublicKey {
        val algorithmIdentifier: AlgorithmIdentifier = AlgorithmIdentifier(PKCSObjectIdentifiers.rsaEncryption, DERNull.INSTANCE);
        val pubKeyX509Bytes: ByteArray = SubjectPublicKeyInfo(algorithmIdentifier, pubKeyPkcs1Bytes).encoded
        val pubKey: java.security.PublicKey = KeyFactory.getInstance("RSA").generatePublic(X509EncodedKeySpec(pubKeyX509Bytes))
        return pubKey
    }
    ...
    fun parseTakerInfoMessage(message: String, keyPair: KeyPair, kmService: KMService) : TakerInfo? {
        ...
        val javaSecTakerKey = kmService.pubKeyFromPkcs1Bytes(takerKeyBytes)
        val takerKey = PublicKey(javaSecTakerKey)
